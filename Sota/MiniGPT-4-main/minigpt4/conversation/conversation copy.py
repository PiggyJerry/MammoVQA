...
class Chat:
    ...
    def answer(self, conv, img_list, **kargs):
        ...
        output_token,logits=output.sequences[0],output.scores
        # print(output_token)
        # for i,token in enumerate(output_token):
            
        #     print(token,self.model.llama_tokenizer.decode(torch.tensor(token), skip_special_tokens=True), F.softmax(logits[i-1][0],dim=-1)[token],torch.max(logits[i-1][0]))
        
        output_text = self.model.llama_tokenizer.decode(output_token, skip_special_tokens=True)
        
        # print(f"yes_token_id: {_yes_token_id}, token: {self.model.llama_tokenizer.convert_ids_to_tokens(_yes_token_id)}")
        # print(f"Yes_token_id: {_Yes_token_id}, token: {self.model.llama_tokenizer.convert_ids_to_tokens(_Yes_token_id)}")
    ...
    def model_generate(self, *args, **kwargs):
        ...
        with self.model.maybe_autocast():
            output = self.model.llama_model.generate(*args, output_scores=True,return_dict_in_generate=True,**kwargs)
        return output
