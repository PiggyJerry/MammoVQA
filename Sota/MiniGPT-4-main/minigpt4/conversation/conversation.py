...
class Chat:
    ...
    def answer(self, conv, img_list, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
        ...
        output_token, logits = outputs.sequences, logits.scores
    ...
    def batch_answer(self, image_list, question_list, chat_list, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9, repetition_penalty=1.0, length_penalty=1, temperature=1.0, max_length=2000):
        ...
        batch_ori_outputs = []
        batch_logits=[]
        # print(outputs.sequences.shape,outputs.scores.shape)
        for output_token, logits in zip(outputs.sequences,torch.stack(outputs.scores).transpose(0,1)):
            if output_token[0] == 0:  # the model might output a unknow token <unk> at the beginning. remove it
                output_token = output_token[1:]
            if output_token[0] == 1:  # some users find that there is a start token <s> at the beginning. remove it
                output_token = output_token[1:]
            ...
            output_text = output_text.split('###')[0]  # remove the stop sign '###'
            output_text = output_text.split('Assistant:')[-1].strip()
            first_index = (output_token == 2277).nonzero(as_tuple=True)[0]
            if len(first_index)!=0:
                first_index=first_index[0].item()
                output_token = output_token[:first_index]
                logits = logits[:first_index]
            batch_outputs.append(output_text)
            batch_logits.append(logits)
            batch_ori_outputs.append(output_token)
        return batch_outputs, batch_logits,batch_ori_outputs