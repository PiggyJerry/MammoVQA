...
class MultiLLaMAForCausalLM(nn.Module):
    ...
    def generate(self, lang_x,vision_x):
        ...
        with torch.no_grad():
            ...
            # print('input_embedding:',input_embedding.device)
            # input_embedding=input_embedding.to(self.lang_model.lm_head.weight.device)
            # print('input_embedding:',input_embedding.device, input_embedding)
            generation = self.lang_model.generate(inputs_embeds = input_embedding, max_new_tokens =200,top_k=50,output_scores=True,return_dict_in_generate=True)
        return generation
