...
def combine_and_preprocess(qas_question,image_list,image_padding_tokens):
    ...
    # cs_new_qestions = [_ for _ in cs_question]
    padding_index = 0
    for img in image_list:
        ...
        # cs_new_qestions[position] = "<image>"+ image_padding_tokens[padding_index] +"</image>" + cs_new_qestions[position]
        padding_index +=1
    
    vision_x = torch.cat(images,dim = 1).unsqueeze(0) #cat tensors and expand the batch_size dim
    qas_text = ''.join(qas_new_qestions) 
    # cs_text = ''.join(cs_new_qestions) 
    ...
    with torch.no_grad():
        for images, qas_questions, img_ids in tqdm(eval_dataloader):
            image =[
                    {
                        'img_path': images[0][i],
                        'position': i  # 将 position 设置为当前索引 i
                    }
        ...
        with open(base_dir+'/Result/RadFM-Exam.json', 'w') as f:
